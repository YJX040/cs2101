...
try:
    intermediate_df = pd.read_csv('./model_csv/louvre_partial.csv')
    ...
    # 尝试跳过错误的行并加载数据
    with open('./model_csv/louvre_partial.csv', 'r', encoding='utf-8') as f:
        lines = f.readlines()
        try:
...
for index, row in df.iterrows():
                ...
                try:
                    ...
                    if detected_language is not None and detected_language.lang == source_language:
                        ...
                        if not translated_text:
                            # 法语翻译失败，尝试英语
                            translated_text = translate_with_retry(text_to_translate, translator, 'en', target_language)
                            if not translated_text:
                                # 英语翻译失败，自动检测语言
                                ...
                                if not translated_text:
                                    # 自动检测语言翻译失败，使用拼音翻译
                                    translated_text = translate_with_retry(text_to_translate, translator, 'zh-cn', target_language)
                        if translated_text:
    ...
    # 在每次迭代后将结果写入文件
    if index == start_index and header:
        df.iloc[start_index:index + 1].to_csv('./model_csv/louvre_partial.csv', mode='a', index=False, encoding='utf-8-sig')
    else:
        df.iloc[[index]].to_csv('./model_csv/louvre_partial.csv', mode='a', index=False, header=False, encoding='utf-8-sig')
    if translation_status == 1:
        if os.path.exists('./model_csv/louvre.csv'):
            df.iloc[[index]].to_csv('./model_csv/louvre.csv', mode='a', header=False, index=False, encoding='utf-8-sig')
        else:
            df.iloc[[index]].to_csv('./model_csv/louvre.csv', mode='a', header=True, index=False, encoding='utf-8-sig')

    # 更新正常结束状态文件
    pd.DataFrame([normal_termination], columns=['normal_termination']).to_csv('./model_csv/normal_louvre.csv', index=False)

# 写入正常结束标志
if normal_termination:
    with open('./model_csv/louvre.txt', 'w') as f:
        f.write('1')
else:
    with open('./model_csv/louvre.txt', 'w') as f:
        f.write('0')
