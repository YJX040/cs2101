...
def import_properties_to_neo4j(csv_file):
    ...
    with open(csv_file, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            object_name = row['Material']
            
            # 检查数据库中是否已存在具有相同值的节点
            subject_node = graph.nodes.match("Collection", name=subject_name).first()
            object_node = graph.nodes.match("Material", name=object_name).first()
            
            if subject_node is None:
                subject_node = Node("Collection", name=subject_name)
                graph.create(subject_node)
            
            if object_node is None:
                object_node = Node("Material", name=object_name)
...
# 示例用法
csv_file = './triples/material_triples.csv'
